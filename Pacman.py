...
class Pacman:
    """
    This is a class for Pacman objects.

    Attributes:
        position (list): x and y coordinates of Pacman's position
        facing (string): direction Pacman is facing
    """
    def __init__(self):
        """
        Constructor method 
...
def main():
    ...
    while(is_valid):
        input_str = input("\nPlease enter a command for your Pacman: ").upper()
        valid_facing = False
        valid_input = False
        ...
        if input_str == "END":
            ...
            for cmd in cmds:
                if input_str == cmd:
                    print("PLACE the Pacman before the {} command".format(input_str))
